...
- Music: `mw1.mp3` or `mw2.mp3` from `newassets/custom/` (loaded via `audio.play_music_file(path)` — loops indefinitely until Play is pressed)
...
- **Shop inventory hover tooltip:** A description panel always appears above the shop when any item is in focus — from either input method. Tooltip priority (highest to lowest): (1) mouse hovering an inventory slot with an item, (2) mouse hovering a shop item, (3) gamepad cursor on the inventory panel, (4) gamepad cursor on the shop panel. Each level is checked with `if not tooltip_lines` so an empty-slot mouse hover falls through to the gamepad cursor rather than suppressing it. This fixed two bugs: the original `gamepad_panel == "shop"` default that blocked inventory hover tooltips entirely, and a follow-up regression where moving the mouse over any empty inventory slot silenced the gamepad cursor tooltip.
